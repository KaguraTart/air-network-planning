...
class LowAltitudeRouteNetwork:
    ...
    def select_area(self, gdf, area_size_m=5000, center_point=None):
        ...
        参数:
        - gdf: 建筑物GeoDataFrame
        - area_size_m: 区域大小（米），默认5000m x 5000m (5km x 5km)
        - center_point: 中心点坐标 [经度, 纬度]，如果为None则选择建筑物最密集的区域
        
        返回:
        - 选取的子区域GeoDataFrame
        """
        print(f"\n=== 选取 {area_size_m/1000:.1f}km x {area_size_m/1000:.1f}km 的区域 ===")
        ...
        if center_point is None:
            ...
            try:
                ...
                # 计算核密度
                kde = KernelDensity(bandwidth=500).fit(points)  # 带宽设为500米
                ...
                print("sklearn未安装，使用几何中心作为区域中心点")
        ...
        print(f"在 {area_size_m/1000:.1f}km x {area_size_m/1000:.1f}km 的区域内找到 {len(selected_buildings)} 个建筑物")
...
if __name__ == "__main__":
    ...
    try:
        if shp_path.exists():
            # 使用5km x 5km的区域进行规划
            # 可以通过center_point参数指定中心点[经度, 纬度]，例如center_point=[120.15, 30.25]
            # 如果不指定center_point，将自动选择建筑物最密集的区域
            net.load_buildings_from_file(
                shp_path, 
                use_small_area=True, 
                area_size_m=5000,  # 5km x 5km 区域
                center_point=None  # 自动选择建筑物最密集的区域
            )
        else:
